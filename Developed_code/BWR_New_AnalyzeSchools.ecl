...
// My addition to code
StandardizedScores := RECORD
  ByState.State;
  DECIMAL4 Score := 100 - (100*(ByState.AveSTRatio - MIN(ByState, AveSTRatio))/(MAX(ByState, AveSTRatio) - MIN(ByState, AveSTRatio)));
  END;
  
  FinalScores := TABLE(ByState, StandardizedScores,State);
  
 OUTPUT(FinalScores, NAMED('Scores'));