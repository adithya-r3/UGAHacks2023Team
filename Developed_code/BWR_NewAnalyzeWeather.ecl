...
OUTPUT(Storm,NAMED('Storm'));
OUTPUT(StFIPS,NAMED('StFIPS'));
...
ScoreRec := RECORD
  sumEventTbl.State;
  Score := (((sumEventTbl.EvtSum*0.2) + (sumEventTbl.InjSum*0.3) + (sumEventTbl.FatSum*0.5))*100);
END;
  
  scoreTbl := TABLE(sumEventTbl,ScoreRec,State);
...
END;
  ...
  scoreTbl2 := TABLE(scoreTbl,ScoreRec1,State);
